 ...
 * @param {Object} mockedAnswers - an object where keys are the
...
exports.mockPrompt = function(generator, mockedAnswers, options) {
  var promptModule = generator.env.adapter.promptModule;
  mockedAnswers = mockedAnswers || {};
  generator.env.adapter._mockedPrompt = generator.env.adapter.prompt;
  generator.env.adapter.prompt = function (questions, answers) {
    this._mockedPrompt(questions, {...answers, ...mockedAnswers });
  }
  var InquirerDummyPrompt = adapter.InquirerDummyPrompt;

  Object.keys(promptModule.prompts).forEach(function(name) {
    promptModule.registerPrompt(
      name,
      InquirerDummyPrompt.bind(InquirerDummyPrompt, answers, options)
    );
  });
};
